...
| **Sample.Vmc** | TCP SDCP `STATget` (one shot); optional `--sdcp-unit`, `--vmc-item` | `dotnet run --project samples/Sample.Vmc -- <host> <STATget-field>` |
...
| **Sample.BroadcastControl** | Interactive REPL: **one long-lived TCP** session, repeated `get` / `set` (VMC only — **no** UDP in this sample); optional `--sdcp-unit`, `--vmc-item` | `dotnet run --project samples/Sample.BroadcastControl -- <host>` — see [Sample.BroadcastControl/README.md](Sample.BroadcastControl/README.md) |
